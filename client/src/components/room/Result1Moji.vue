...
<template>
  <div class="w-full h-156 bg-background">
    <p class="text-primary text-6xl ml-48">{{ resultData.count.toLocaleString('ja-JP-u-nu-hanidec', { useGrouping: false}) }}文字目</p>
    <div class="flex flex-row gap-24">
      <div class="w-128 h-128 bg-white mt-8 ml-48"></div>
      <!--キャンバス代わりの白正方形-->
      <!--ここにキャンバス-->
      <div class="flex flex-col">
        <p class="text-primary text-8xl">推測{{ resultData.actualAnswer === resultData.guesserAnswer ? '成功' : '失敗' }}</p>
